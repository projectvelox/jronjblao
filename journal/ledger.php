...
<?php 
    ...
    $query_main = "SELECT *,
                    ...
                    FROM journal_main 
                    WHERE j_customer_id=$customer_id
                    ORDER BY j_transaction_date ASC, j_id ASC";
    ...
    $data .= ']';  
?>
<div id="dbgrid-ledger"></div>
<script>
    $("#dbgrid-ledger").jsGrid({
        height: "100%",
        width: "100%",
        filtering: false,
        editing: false,
        ...
        fields: [
            {   
                title:"", align:"left",width:"30px", align:"center", sorting:false,
                itemTemplate: function(value,e) { 
                    if(e.j_status=="A"){
                        return $("<div>").append('<i class="fa fa-check" aria-hidden="true"></i>'); 
                    } else {
                        return $("<div>").append('<input type="radio" name="selected-record" value="'+e.j_id+'">'); 
                    }
                }, 
            },    
            {   
