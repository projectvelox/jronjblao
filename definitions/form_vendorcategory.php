<?php
    ...
    $data = array();
    if($_GET["f"]=="edit") {
            $query = "SELECT * FROM vendors_category WHERE cat_id=".$_GET['id'];
...
<div class="form-container" height="262" width="450" uuid="">
    <form id="form-definition">   
        <input name="db" type="hidden" value="vendorcategory">
        <input name="record_id" type="hidden" value="<?=$data[0]["cat_id"];?>">
        <span class="label-block">Category</span>
        <input name="cat_name" type="text" value="<?=$data[0]["cat_name"];?>" class="field-input-block">
        <span class="label-block">Description</span>
        <textarea name="cat_desc" rows="4" cols="50" class="field-textarea-block "><?=$data[0]["cat_desc"];?></textarea>
    </form>     
</div>
