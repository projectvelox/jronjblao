...
<?php 
    ...
    $query = "SELECT *,
               ...
               FROM inventory WHERE p_status='D' OR p_status='L' AND p_branch_id=".$_SESSION['branch_id']." ORDER BY p_recieved_date ASC";
    $recordset = mysql_query($query) or die('Query failed: ' . mysql_error());
    $data = '[';
    while ($row = mysql_fetch_array($recordset, MYSQL_ASSOC)) {
        $status ="";
        if($row['p_status']=="L") { $status = "Lost"; } 
        else if ($row['p_status']=="D")  { $status = "Damage"; } 
        $data .= '{
            "p_id":"'.$row['p_id'].'",
            "p_stockno":"'.get_stockcode($row['p_id']).'-'.$row['p_id'].'",
            "p_product_id":"'.$row['p_product_id'].'",
            "p_reported":"'.date('m/d/Y',strtotime($row['p_reported'])).'",
...
<script>
    $("#dbgrid").jsGrid({
        ...
        fields: [
            ...
            {name:"p_reported", type:"text", width:"100px", title:"REPORTED", align:"center"},
            {name:"status", type:"text", width:"80px", title:"STATUS", align:"left"},
            {   
                title:"STOCKNO", align:"left",width:70, align:"left", sorting:true,
                itemTemplate: function(value,e) { return $("<div>").append('<div><span data-id="'+e.p_id+'" class="grid-link stock-view">'+e.p_stockno+'</span></div>'); },                
            }, 
