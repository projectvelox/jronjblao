...
<?php 
    ...
    $query = "SELECT *,
                ...
                (SELECT b_name FROM branches WHERE branches.b_id=inventory.p_branch_id) AS p_branch                FROM inventory 
                    ...
                    AND p_branch_id=".$_SESSION['branch_id']." ORDER BY p_recieved_date ASC";
    $recordset = mysql_query($query) or die('Query failed: ' . mysql_error());
    $data = '[';
    while ($row = mysql_fetch_array($recordset, MYSQL_ASSOC)) {
        $status ="";
        if($row['p_status']=="R") { $status = "Returned"; } 
        else { $status = ""; } 
        $data .= '{
            "p_id":"'.$row['p_id'].'",
            "p_stockno":"'.get_stockcode($row['p_id']).'-'.$row['p_id'].'",
            "p_product_id":"'.$row['p_product_id'].'",
            "p_returned_dated":"'.date('m/d/Y',strtotime($row['p_returned_dated'])).'",
...
<script>
    $("#dbgrid").jsGrid({
        ...
        fields: [
            ...
            {name:"p_returned_dated", type:"text", width:"100px", title:"RETURNED", align:"center"},
