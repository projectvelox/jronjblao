    ...
    <form id="form_sales"> 
        ...
        <div class="col-lg-8">
            ...
            <div class="col-lg-4">
                ...
                <input name="s_tax" type="number" value="<?=$data_sales[0]['s_tax'];?>" class="field-input-block-thin">
            </div>
            <div class="col-lg-4">
                <span class="label-block align-text">Other Charges</span>
                <input name="s_othercharges" type="number" value="<?=$data_sales[0]['s_othercharges'];?>" class="field-input-block-thin">
            </div>
            <div class="col-lg-4">
                <span class="label-block align-text">Discount</span>
                <input name="s_discount" type="number" value="<?=$data_sales[0]['s_discount'];?>" class="field-input-block-thin">
            </div>
            <div class="col-lg-12">
                <span class="label-block">Notes</span>
                <textarea name="s_notes" rows="4" cols="50" class="textarea-remarks-input" style="height: 204px"><?=$data_sales[0]['s_notes'];?></textarea>
            </div> 
        </div>        
        <div class="col-lg-4">     
            <div class="col-lg-7"><span class="label-block align-text">Terms</span></div>
            <div class="col-lg-5">
                <select name="s_payment_term" id="s_payment_term" class="field-input-block-thin">
                <?php
                    $query = "SELECT * FROM paymentterms ORDER BY t_code ASC";
                    $recordset = mysql_query($query) or die('Query failed: ' . mysql_error());
                    while ($row = mysql_fetch_array($recordset, MYSQL_ASSOC)) {
                        if($row['t_terms']==$data_sales[0]['s_payment_term']) {
                            echo "<option value='".$row['t_terms']."' selected>".$row['t_code']."</option>";   
                        } else {
                            echo "<option value='".$row['t_terms']."'>".$row['t_code']."</option>";   
                        }
                    }
                ?>
                </select>
            </div>
            <div class="col-lg-7"><span class="label-block align-text">Downpayment Type</span></div>
            <div class="col-lg-5">
                <select name="s_downpayment_type" id="s_downpayment_type" class="field-input-block-thin">
                    <?php
            ...
            <div class="col-lg-5"><input name="s_firstmonthlydue_date" type="text" value="<?=date('m/d/Y',strtotime('+45 days'));?>" class="field-input-block-thin"></div>
            <div class="col-lg-7"><span class="label-block align-text">First Downpayment</span></div>
            <div class="col-lg-5"><input name="s_firstdownpayment" type="number" value="0" class="field-input-block-thin"></div>
            <div class="col-lg-7"><span class="label-block align-text">Due Date</span></div>
            <div class="col-lg-5"><input name="s_firstdownpayment_date" type="text" value="<?=date('m/d/Y',strtotime('+15 days'));?>" class="field-input-block-thin"></div>
            <div class="col-lg-7"><span class="label-block align-text">Second Downpayment</span></div>
            <div class="col-lg-5"><input name="s_seconddownpayment" type="number" value="0" class="field-input-block-thin"></div>
            <div class="col-lg-7"><span class="label-block align-text">Due Date</span></div>
            <div class="col-lg-5"><input name="s_seconddownpayment_date" type="text" value="<?=date('m/d/Y',strtotime('+30 days'));?>" class="field-input-block-thin"></div>
            <div class="col-lg-7"><span class="label-block align-text">Monthly</span></div>
            <div class="col-lg-5"><input disabled name="s_monthlyamortization" type="number" value="" class="field_readonly_block"></div>
...
<div class="form-footer">
    ...
    <button closewindow="off" class="form-button update-sales-nonsp-installment"><i class="fa fa-plus" aria-hidden="true"></i> Update</button>
