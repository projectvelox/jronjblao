...
<?php 
    ...
    $query_main = "SELECT *,
                        ...
                        (SELECT CONCAT(e_lastname,', ',e_firstname) FROM employees WHERE employees.e_id=stocktransfer_main.t_verifiedby_id) AS t_verifiedby
                     FROM stocktransfer_main 
                     WHERE 
                        t_approved_date IS NULL   
                        AND t_intransit_date IS NULL   
                        AND t_received_date IS NULL   
                        AND t_cancel_date IS NULL   
                        AND t_verify_date IS NOT NULL   
    ...
    while ($row_main = mysql_fetch_array($recordset_main, MYSQL_ASSOC)) {
        $data .= '{
            ...
            "t_verify_date":"'.date('m/d/Y',strtotime($row_main['t_verify_date'])).'",
            "t_branch_origin":"'.$row_main['t_branch_origin'].'",
            "t_branch_destination":"'.$row_main['t_branch_destination'].'",
            "t_verifiedby":"'.$row_main['t_verifiedby'].'",
...
<script>
    $("#dbgrid").jsGrid({
        ...
        fields: [
            ...
            {name:"t_verify_date", type:"text", width:"100px", title:"VERIFIED", align:"center"},
            ...
            {name:"t_verifiedby", type:"text", width:"150px", title:"VERIFIED BY", align:"left"},
