...
<?php 
    ...
    $query_main = "SELECT *,
                        ...
                        (SELECT CONCAT(e_lastname,', ',e_firstname) FROM employees WHERE employees.e_id=stocktransfer_main.t_receivedby_id) AS t_receivedby
                     FROM stocktransfer_main 
                     WHERE 
                        t_cancel_date IS NULL   
                        AND t_received_date IS NOT NULL   
    ...
    while ($row_main = mysql_fetch_array($recordset_main, MYSQL_ASSOC)) {
        $data .= '{
            ...
            "t_received_date":"'.date('m/d/Y',strtotime($row_main['t_received_date'])).'",
            "t_branch_origin":"'.$row_main['t_branch_origin'].'",
            "t_branch_destination":"'.$row_main['t_branch_destination'].'",
            "t_receivedby":"'.$row_main['t_receivedby'].'",
    ...
    $("#dbgrid").jsGrid({
        ...
        fields: [
            ...
            {   
                ...
                itemTemplate: function(value,e) { return $("<div>").append('<div><span data-id="'+e.t_id+'" class="grid-link transfer-view-recieved">'+e.t_no+'</span></div>'); },                
            }, 
            {name:"t_received_date", type:"text", width:"100px", title:"RECEIVED", align:"center"},
            ...
            {name:"t_receivedby", type:"text", width:"150px", title:"RECEIVED BY", align:"left"},
