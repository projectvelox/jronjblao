...
<?php 
    ...
    $query_main = "SELECT *,
                        ...
                        (SELECT CONCAT(e_lastname,', ',e_firstname) FROM employees WHERE employees.e_id=stocktransfer_main.t_approvedby_id) AS t_approvedby
                     FROM stocktransfer_main 
                     WHERE 
                        t_intransit_date IS NULL   
                        AND t_received_date IS NULL   
                        AND t_cancel_date IS NULL
                        AND t_approved_date IS NOT NULL
    ...
    while ($row_main = mysql_fetch_array($recordset_main, MYSQL_ASSOC)) {
        $data .= '{
            ...
            "t_approved_date":"'.date('m/d/Y',strtotime($row_main['t_approved_date'])).'",
            "t_branch_origin":"'.$row_main['t_branch_origin'].'",
            "t_branch_destination":"'.$row_main['t_branch_destination'].'",
            "t_approvedby":"'.$row_main['t_approvedby'].'",
...
<script>
    $("#dbgrid").jsGrid({
        ...
        fields: [
            ...
            {name:"t_approved_date", type:"text", width:"100px", title:"APPROVED", align:"center"},
            ...
            {name:"t_approvedby", type:"text", width:"150px", title:"APPROVED BY", align:"left"},
