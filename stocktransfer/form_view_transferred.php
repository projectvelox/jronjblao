...
<?php
    ...
    while($row_main = mysql_fetch_assoc($recordset_main)){
        ...
        if($row_main['t_verify_date'] != "") { $status = "Verified";}
        if($row_main['t_approved_date'] != "") { $status = "Approved"; }
        if($row_main['t_intransit_date'] != "") { $status = "In Transit"; }
        if($row_main['t_received_date'] != "") { $status = "Received"; }
        if($row_main['t_cancel_date'] != "") { $status = "Cancelled"; }
    ...
    $query = "SELECT *, 
                ...
                (SELECT p_name FROM products WHERE products.p_id=(SELECT p_product_id FROM inventory WHERE inventory.p_id=stocktransfer_sub.p_id)) AS p_name
                FROM stocktransfer_sub WHERE t_id=".$_GET['id'];
    ...
    while ($row = mysql_fetch_array($recordset, MYSQL_ASSOC)) {
        $data_tranferitems .= '{
            ...
            "p_code":"'.$row['p_code'].'",
            "p_name":"'.$row['p_name'].'",
            "p_is_recieved":"'.$row['p_is_recieved'].'",
        },';
    }
    $data_tranferitems .= ']';  
...
<div class="form-container bg-lightgray" height="536" width="800" uuid="">
    <form id="form_newtransferrequest"> 
        ...
        <div class="col-lg-4">
            ...
            <input disabled name="t_request_date" type="text" value="<?=formatdate($data[0]["t_request_date"]);?>" class="field_readonly_block">
        </div>
        <div class="col-lg-4">
        ...
        <div class="col-lg-6">
            ...
            <select disabled ame="t_branch_destination_id" id="t_branch_destination_id" class="field_readonly_block">
            <?php
                echo "<option value='' selected></option>";
                $query = "SELECT * FROM branches ORDER BY b_name ASC";
                $recordset = mysql_query($query) or die('Query failed: ' . mysql_error());
        ...
        <div class="col-lg-12">
            ...
            <textarea disabled name="t_notes" rows="4" cols="50" class="textarea-remarks-readonly"><?=$data[0]["t_notes"];?></textarea>
        </div> 
        <div class="col-lg-12">
...
<script>
    $("#dbgrid-tranferitems").jsGrid({
        ...
        fields: [           
            ...
            {   
                title:"TRANSFERRED", align:"left",width:"70px", align:"center", sorting:false,
                itemTemplate: function(value,e) { 
                    if(e.p_is_recieved=="Y"){
                        return $("<div>").append('<i class="fa fa-check" aria-hidden="true"></i>'); 
                    } else {
                        return $("<div>").append('<button data-id="'+e.t_sub_id+'" closewindow="off" class="form-button-recieved stock-transfer-recieve">Recieve</button>'); 
                    }
                },
            }, 
