...
<?php 
    ...
    $query_main = "SELECT *,
                        ...
                        (SELECT CONCAT(e_lastname,', ',e_firstname) FROM employees WHERE employees.e_id=stocktransfer_main.t_intransitby_id) AS t_intransitby
                     FROM stocktransfer_main 
                     WHERE 
                        t_received_date IS NULL   
                        AND t_cancel_date IS NULL 
                        AND t_intransit_date IS NOT NULL
    ...
    while ($row_main = mysql_fetch_array($recordset_main, MYSQL_ASSOC)) {
        $data .= '{
            ...
            "t_intransit_date":"'.date('m/d/Y',strtotime($row_main['t_intransit_date'])).'",
            "t_branch_origin":"'.$row_main['t_branch_origin'].'",
            "t_branch_destination":"'.$row_main['t_branch_destination'].'",
            "t_intransitby":"'.$row_main['t_intransitby'].'",
    ...
    $("#dbgrid").jsGrid({
        ...
        fields: [
            ...
            {   
                ...
                itemTemplate: function(value,e) { return $("<div>").append('<div><span data-id="'+e.t_id+'" class="grid-link transfer-view-intransit">'+e.t_no+'</span></div>'); },                
            }, 
            {name:"t_intransit_date", type:"text", width:"100px", title:"IN TRANSIT", align:"center"},
            ...
            {name:"t_intransitby", type:"text", width:"150px", title:"CONFIRMED BY", align:"left"},
